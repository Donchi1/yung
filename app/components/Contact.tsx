...
function Contact() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      email: Yup.string().email().trim().lowercase().required("Email Field is required")
    }),
  ...
  const { values, errors, handleSubmit, getFieldProps, setFieldValue, isSubmitting, touched } = formik
        ...
        <div className='flex items-center gap-14 flex-col lg:flex-row '>
          <div className='flex-1 space-y-10 '>
            <div className='space-y-4'>
              ...
              <Text className='!text-light-color/70' >Contact us for more information about our products and services.Reach out to us if you have any question.</Text>
            </div>
            <hr color='gray' />
            <div className='space-y-6'>
              <div className="flex items-center gap-4">
                ...
                <div className='space-y-4'>
                  <Text>Our Phone</Text>
                  <a className='text-light-color/70 hover:text-primary-color transition-all duration-500 ease-linear' href='tel:+5637278398276'>+5637278398276</a>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <span className='inline-flex justify-center items-center w-12 h-12 rounded-full bg-primary-color'>
                ...
                <div className='space-y-4'>
                  <Text>Our Email</Text>
                  <a className='text-light-color/70 hover:text-primary-color transition-all duration-500 ease-linear' href='mailTo:support@yungglobal.pro'>support@yungglobal.pro</a>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <span className='inline-flex justify-center items-center w-12 h-12 rounded-full bg-primary-color'>
                ...
                <div className='space-y-4'>
                  ...
                  <span className='text-light-color/70 hover:text-primary-color transition-all duration-500 ease-linear'>359 E Corona Rd, Tucson, AZ 85756, United States</span>
                </div>
              </div>
          ...
          <div className="flex-1 contact-bg  h-screen " >
            <form className='px-4 lg:px-14 grid grid-cols-2 grid-rows-subgrid gap-8 content-center h-screen' onSubmit={handleSubmit}>
              ...
              <Input className='' placeholder='Subject' error={touched.subject && errors.subject}  {...getFieldProps("subject")} value={values.subject} />
              <div className='col-start-1 col-end-3 '>
              <TextArea  rows={4} cols={3} placeholder='Write a message' error={touched.message && errors.message}  {...getFieldProps("message")} value={values.message} />
              </div>
              <div className='col-start-1 col-end-3 '>
              <PrimaryButton disabled={isSubmitting} className='!w-full' title='Contact Us' />
              </div>
